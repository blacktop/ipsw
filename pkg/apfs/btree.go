...
func (a *APFS) GetOMapEnties(root *types.BTreeNodePhys, entCount uint16) ([]types.OMapNodeEntry, error) {
	var entries []types.OMapNodeEntry

	var ent types.OMapNodeEntry
	if err := binary.Read(a.r, binary.LittleEndian, &ent.Offset); err != nil {
		return nil, fmt.Errorf("failed to read omap entry offset: %v", err)
	}

	// pos, _ := a.r.Seek(0, io.SeekCurrent)

	// a.r.Seek(int64(root.Block.r))
